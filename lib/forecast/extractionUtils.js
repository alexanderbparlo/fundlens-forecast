...
// Server-side file size guard: base64 strings should not exceed ~14MB
// (10MB binary * 4/3 base64 expansion, with margin).
const MAX_BASE64_CHARS = 14_500_000
...
export function validateFilePayload(file, label) {
  ...
  if (file.dataType === 'base64' && typeof file.data === 'string' && file.data.length > MAX_BASE64_CHARS) {
    return `${label} file exceeds the 10 MB size limit`
  }
  return null
}
